...
import datetime
from ..items import MagicbrickssalehyderabadItem
from datetime import datetime as dt
import re
from scrapy.http import Request


class MagicsalehyderabadSpider(scrapy.Spider):
    name = "Magicbricks_SaleHyderabad"
    allowed_domains = ["magicbricks.com"]
    start_urls = [
        'http://www.magicbricks.com/property-for-sale/residential-real-estate?proptype=Multistorey-Apartment,Builder-Floor-Apartment,Penthouse,Studio-Apartment,Residential-House,Villa&cityName=Hyderabad&sortBy=mostRecent/Page-1',
    ]
    custom_settings = {
        'DEPTH_LIMIT': 100000,
        'DOWNLOAD_DELAY': 5.0,
    ...
    def parse(self, response):
        try:
            item = MagicbrickssalehyderabadItem()
                ...
                try:
                    ...
                    try:
                        ...
                        if 'sqft' in sqf:
                            ...
                            item['price_per_sqft'] = str(eval(item['price_per_sqft']) / 10)
                        elif 'sqyrd':
                            item['Bua_sqft'] = re.findall('[0-9]+', sqf)
                            item['Bua_sqft'] = str(eval(item['Bua_sqft'][0]) * 9)
                            item['price_per_sqft'] = str(eval(item['price_per_sqft']) / 9)
                        elif 'acre' in sqf:
                            item['Bua_sqft'] = re.findall('[0-9]+', sqf)
                            item['Bua_sqft'] = str(eval(item['Bua_sqft'][0]) * 43560)
                            item['price_per_sqft'] = str(eval(item['price_per_sqft']) / 43560)
                        else:
                            item['Bua_sqft'] = '0'
                    except:
                        item['Bua_sqft'] = '0'
