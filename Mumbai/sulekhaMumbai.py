...
class MySpider(CrawlSpider):
	...
	def parse1(self , response):
		...
		self.item['locality'] = hxs.xpath('//input[@id="d_locality"]/@value').extract_first().replace(',',';')
		...
		if 'Builder' in hxs.xpath('//div[@class="page-details-info"]/i[2]/text()').extract_first():
			self.item['listing_by'] = hxs.xpath('//div[@class="page-details-info"]/text()').extract()[2].replace(',',' ').replace('\n',' ').replace('\t',' ')
		else:
			self.item['name_lister'] = hxs.xpath('//div[@class="page-details-info"]/text()').extract()[2].replace(',',' ').replace('\n',' ').replace('\t',' ')
